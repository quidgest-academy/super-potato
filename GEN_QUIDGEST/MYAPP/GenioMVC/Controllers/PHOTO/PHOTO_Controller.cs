...
namespace GenioMVC.Controllers
{
	public partial class PhotoController : ControllerBase
	{
		...
		public PhotoController(UserContextService userContext, IChatbotService aiService) : base(userContext)
		{
			_aiService = aiService;
		}

// USE /[MANUAL FOR CONTROLLER_NAVIGATION PHOTO]/



		private List<string> GetActionIds(CriteriaSet crs, CSGenio.persistence.PersistentSupport sp = null)
		{
			CSGenio.business.Area area = CSGenio.business.Area.createArea<CSGenioAphoto>(UserContext.Current.User, UserContext.Current.User.CurrentModule);
			return base.GetActionIds(crs, sp, area);
		}

// USE /[MANUAL FOR MANUAL_CONTROLLER PHOTO]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			Models.Photo row = new Models.Photo(UserContext.Current, isEmpty: true);
			row.klass.QPrimaryKey = Navigation.GetStrValue("photo");
			...
			try
			{
				...
				{
					case "ALBUM___PROPETITLE___":	// Field (DB)
						{
							var model = new Album_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_Album___propetitle___(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "ALBUM___PROPETITLE___":	// Field (DB)
						values = new Album_ViewModel(UserContext.Current).GetDependant_AlbumTablePropeTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "Album" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_Album([FromBody]Album_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "photo",
				(primaryKey) => Models.Photo.Find(primaryKey, UserContext.Current, "FALBUM"),
				(model) => formData.MapToModel(model as Models.Photo)
			);
		}
		...
		{
			return base.GetDocumsTickets("PHOTO", requestModel.FieldName, requestModel.KeyValue);
		}
