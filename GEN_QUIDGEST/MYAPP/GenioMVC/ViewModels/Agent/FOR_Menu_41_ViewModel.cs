...
{
	public class FOR_Menu_41_ViewModel : MenuListViewModel<Models.Agent>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<FOR_Menu_41_RowViewModel> Menu { get; set; }
...
// USE /[MANUAL FOR LIST_LIMITS 41]/
		...
		public FOR_Menu_41_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="FOR_Menu_41_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public FOR_Menu_41_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public FOR_Menu_41_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<FOR_Menu_41_RowViewModel>();
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAagent> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet for_menu_41Conds = CriteriaSet.And();
			...
			conditions.SubSets.Add(for_menu_41Conds);
			for_menu_41Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL FOR OVERRQ 41]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAagent>(m_userContext, false, ref for_menu_41Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML41", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL FOR OVERRQLSTEXP 41]/
...
// USE /[MANUAL FOR OVERRQLIST 41]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAagent.GetInformation(), QMVC_POS_RECORD, sorts, for_menu_41Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAagent> listing = Models.ModelBase.Where<CSGenioAagent>(m_userContext, distinct, for_menu_41Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML41", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapFOR_Menu_41(listing);
		...
		private List<FOR_Menu_41_RowViewModel> MapFOR_Menu_41(ListingMVC<CSGenioAagent> Qlisting)
		{
			List<FOR_Menu_41_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapFOR_Menu_41(row));
					i++;
				}
			}
		...
		/// to a FOR_Menu_41_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private FOR_Menu_41_RowViewModel MapFOR_Menu_41(CSGenioAagent row)
		{
			var model = new FOR_Menu_41_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL FOR VIEWMODEL_CUSTOM FOR_MENU_41]/
