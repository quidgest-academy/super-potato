...
{
	public class FOR_Menu_411_ViewModel : MenuListViewModel<Models.Prope>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<FOR_Menu_411_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "407f1d81-03ca-4ea0-8450-7b31b3489c06";
		...
		{
// USE /[MANUAL FOR LIST_LIMITS 411]/
		...
		public override int GetCount(User user)
		{
			...
			if (!qs.Joins.Select(x => x.Table).Select(y => y.TableAlias).Contains(CSGenio.business.Area.AreaAGENT.Alias))
				qs.Join(CSGenio.business.Area.AreaAGENT, TableJoinType.Inner).On(CriteriaSet.And().Equal(CSGenioAagent.FldCodagent, CSGenioAprope.FldCodagent));
		...
		public FOR_Menu_411_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="FOR_Menu_411_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public FOR_Menu_411_ViewModel(UserContext userContext) : base(userContext)
		{
		...
		public FOR_Menu_411_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<FOR_Menu_411_RowViewModel>();
			...
			// Limitations
			// Limit "DB"
			crs.Equal(CSGenioAprope.FldCodagent, Navigation.GetValue("agent"));
			if (isToExport)
			{
				// EPH
				crs = Models.Prope.AddEPH<CSGenioAprope>(ref u, crs, "ML411");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Prope.AddEPH<CSGenioAprope>(ref u, null, "ML411"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAprope> Qlisting, ref CriteriaSet conditions)
		{
			...
			CriteriaSet for_menu_411Conds = CriteriaSet.And();
			...
			conditions.SubSets.Add(for_menu_411Conds);
			for_menu_411Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
			tableReload &= hasAllRequiredLimits;

// USE /[MANUAL FOR OVERRQ 411]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAprope>(m_userContext, false, ref for_menu_411Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML411", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL FOR OVERRQLSTEXP 411]/
...
// USE /[MANUAL FOR OVERRQLIST 411]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAprope.GetInformation(), QMVC_POS_RECORD, sorts, for_menu_411Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAprope> listing = Models.ModelBase.Where<CSGenioAprope>(m_userContext, distinct, for_menu_411Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "ML411", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapFOR_Menu_411(listing);
		...
		private List<FOR_Menu_411_RowViewModel> MapFOR_Menu_411(ListingMVC<CSGenioAprope> Qlisting)
		{
			List<FOR_Menu_411_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapFOR_Menu_411(row));
					i++;
				}
			}
		...
		/// to a FOR_Menu_411_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private FOR_Menu_411_RowViewModel MapFOR_Menu_411(CSGenioAprope row)
		{
			var model = new FOR_Menu_411_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL FOR VIEWMODEL_CUSTOM FOR_MENU_411]/
