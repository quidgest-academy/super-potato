...
{
	public class Album_PropeValTitle_ViewModel : MenuListViewModel<Models.Prope>
	{
		/// <summary>
		/// Gets or sets the object that represents the table and its elements.
		/// </summary>
		[JsonPropertyName("table")]
		public TablePartial<Album_PropeValTitle_RowViewModel> Menu { get; set; }
		...
		public override string Uuid => "Album_PropeValTitle";
		...
		{
// USE /[MANUAL FOR LIST_LIMITS ALBUM_PROPETITLE]/
		...
		public Album_PropeValTitle_ViewModel() : base(null!) { }

		/// <summary>
		/// Initializes a new instance of the <see cref="Album_PropeValTitle_ViewModel" /> class.
		/// </summary>
		/// <param name="userContext">The current user request context</param>
		public Album_PropeValTitle_ViewModel(UserContext userContext) : base(userContext)
		{
			ValCodphoto = userContext.CurrentNavigation.CurrentLevel.GetEntry("photo")?.ToString();
		}
		...
		public Album_PropeValTitle_ViewModel(UserContext userContext, Models.ModelBase parentCtx) : this(userContext)
		{
			ParentCtx = parentCtx;
		}
		...
		public override CriteriaSet BuildCriteriaSet(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, out bool tableReload, CriteriaSet crs = null, bool isToExport = false)
		{
			...
			Menu ??= new TablePartial<Album_PropeValTitle_RowViewModel>();
			...
			{
				// EPH
				crs = Models.Prope.AddEPH<CSGenioAprope>(ref u, crs, "IBL_ALBUM___PROPETITLE___");
			...
			if (tableReload)
			{
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
					crs.Equals(Models.Prope.AddEPH<CSGenioAprope>(ref u, null, "IBL_ALBUM___PROPETITLE___"));
			}
		...
		public void Load(CSGenio.core.framework.table.TableConfiguration tableConfig, NameValueCollection requestValues, bool ajaxRequest, bool isToExport, ref ListingMVC<CSGenioAprope> Qlisting, ref CriteriaSet conditions)
		{
			...
			//Tooltip for EPHs affecting this viewmodel list
			{
				...
				List<Limit> area_EPH_limits = EPH_Limit_Filler(ref limit, model_limit_area, "IBL_ALBUM___PROPETITLE___");
			...
			conditions.SubSets.Add(album___propetitle___Conds);
			album___propetitle___Conds = BuildCriteriaSet(tableConfig, requestValues, out bool hasAllRequiredLimits, conditions, isToExport);
...
// USE /[MANUAL FOR OVERRQ ALBUM_PROPETITLE]/
			...
			if (isToExport)
			{
				...
				Qlisting = Models.ModelBase.BuildListingForExport<CSGenioAprope>(m_userContext, false, ref album___propetitle___Conds, exportFieldRefs, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ALBUM___PROPETITLE___", true, firstVisibleColumn: firstVisibleColumn);

// USE /[MANUAL FOR OVERRQLSTEXP ALBUM_PROPETITLE]/
			...
			if (tableReload)
			{
// USE /[MANUAL FOR OVERRQLIST ALBUM_PROPETITLE]/
				...
				if (!string.IsNullOrEmpty(QMVC_POS_RECORD))
				{
					var m_iCurPag = m_userContext.PersistentSupport.getPagingPos(CSGenioAprope.GetInformation(), QMVC_POS_RECORD, sorts, album___propetitle___Conds, m_PagingPosEPHs, firstVisibleColumn: firstVisibleColumn);
					if (m_iCurPag != -1)
						pageNumber = ((m_iCurPag - 1) / numberListItems) + 1;
				}

				ListingMVC<CSGenioAprope> listing = Models.ModelBase.Where<CSGenioAprope>(m_userContext, distinct, album___propetitle___Conds, fields, (pageNumber - 1) * numberListItems, numberListItems, sorts, "IBL_ALBUM___PROPETITLE___", true, false, QMVC_POS_RECORD, m_PagingPosEPHs, firstVisibleColumn, fieldsWithTotalizers, tableConfig.SelectedRows);
				...
				Menu.Elements = MapAlbum_PropeValTitle(listing);

				Menu.Identifier = "IBL_ALBUM___PROPETITLE___";

				// Last updated by [CJP] at [2015.02.03]
				// Adds the identifier to each element
				foreach (var element in Menu.Elements)
					element.Identifier = "IBL_ALBUM___PROPETITLE___";
		...
		private List<Album_PropeValTitle_RowViewModel> MapAlbum_PropeValTitle(ListingMVC<CSGenioAprope> Qlisting)
		{
			List<Album_PropeValTitle_RowViewModel> Elements = [];
			int i = 0;
			...
			if (Qlisting.Rows != null)
			{
				foreach (var row in Qlisting.Rows)
				{
					if (Qlisting.NumRegs > 0 && i >= Qlisting.NumRegs) // Copiado da versão antiga do RowsToViewModels
						break;
					Elements.Add(MapAlbum_PropeValTitle(row));
					i++;
				}
			}
		...
		/// to a Album_PropeValTitle_RowViewModel object.
		/// </summary>
		/// <param name="row">The row.</param>
		private Album_PropeValTitle_RowViewModel MapAlbum_PropeValTitle(CSGenioAprope row)
		{
			var model = new Album_PropeValTitle_RowViewModel(m_userContext, true, _fieldsToSerialize);
			if (row == null)
				return model;
...
// USE /[MANUAL FOR VIEWMODEL_CUSTOM ALBUM_PROPEVALTITLE]/
