<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-city-countcountry-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['CITY____COUNTCOUNTRY__SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'CityCountcountrySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('CITY____COUNTCOUNTRY__SeeMore', false),

				interfaceMetadata: {
					id: 'CITY____COUNTCOUNTRY__SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'CITY____COUNTCOUNTRY__SeeMore',
					name: 'CITY____COUNTCOUNTRY__SeeMore',
					controller: 'CITY',
					action: 'CITY_CountValCountry'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-city-countcountry',
				dismissAction: this.close,
				returnElement: 'CITY____COUNTCOUNTRY__see-more_button'
			}
			const props = {
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-city-countcountry')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						globalEvents: ['changed-COUNT'],
						uuid: 'City_City_CountValCountry',
