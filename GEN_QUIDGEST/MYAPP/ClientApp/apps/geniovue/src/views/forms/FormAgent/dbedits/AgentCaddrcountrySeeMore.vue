<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-agent-caddrcountry-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['AGENT___CADDRCOUNTRY__SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'AgentCaddrcountrySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('AGENT___CADDRCOUNTRY__SeeMore', false),

				interfaceMetadata: {
					id: 'AGENT___CADDRCOUNTRY__SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'AGENT___CADDRCOUNTRY__SeeMore',
					name: 'AGENT___CADDRCOUNTRY__SeeMore',
					controller: 'AGENT',
					action: 'AGENT_CaddrValCountry'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-agent-caddrcountry',
				dismissAction: this.close,
				returnElement: 'AGENT___CADDRCOUNTRY__see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.COUNTRIES_OF_RESIDEN57621),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-agent-caddrcountry')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						config: {
							name: 'Agent_CaddrValCountry',
							serverMode: true,
							pkColumn: 'ValCodcount',
							tableAlias: 'CADDR',
							tableNamePlural: computed(() => this.Resources.COUNTRIES_OF_RESIDEN57621),
							viewManagement: '',
							showLimitsInfo: true,
							tableTitle: '',
						...
						globalEvents: ['changed-CADDR'],
						uuid: 'Agent_Agent_CaddrValCountry',
