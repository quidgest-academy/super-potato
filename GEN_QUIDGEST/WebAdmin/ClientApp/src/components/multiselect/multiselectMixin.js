...
export default {
	...
	methods: {
		...
		isSelected(option) {
			...
			return (
				(typeof opt === 'object'
					? this.valueKeys.findIndex(function (vk) {
							return this.compareValues(vk['Value'], opt['Value'])
						})
					: this.valueKeys.indexOf(opt)) > -1
			)
		},
		/**
		...
		selectGroup(selectedGroup) {
			...
			if (this.wholeGroupSelected(group)) {
				...
				const optionsToAdd = group[this.groupValues].filter(
					not(this.isOptionDisabled || this.isSelected)
				)

				this.$emit('select', optionsToAdd, this.id)
				this.$emit('update:modelValue', this.internalValue.concat(optionsToAdd), this.id)
			}
		},
		/**
			...
			const index =
				typeof option === 'object'
					? this.trackBy
						...
						: this.valueKeys.findIndex(function (vk) {
								return this.compareValues(vk['Value'], option['Value'])
							})
